			   ...
			   .equ		 ZERO,				0b00111111
			   .equ		 ONE,				0b0000011000000000 
			   .equ		 TWO,				0b010110110000000000000000 
			   .equ		 THREE,				0b01001111000000000000000000000000 
/*********************************************************************************
...
KEY_ISR:
                LDR      R0, =0xFF200050		 // pushbutton KEY base address
				LDR		 R1, =0xFF200020		 // HEX[3:0] base address
				LDR		 R2, [R0, #0xC]
				
				CMP		 R2, #0b1				 // check for KEY[0]
				MOVEQ	 R3, #ZERO
				
				CMP		 R2, #0b10				 // check for KEY[1]
				MOVEQ	 R3, #ONE
				
				CMP		 R2, #0b100				 // check for KEY[2]
				MOVEQ	 R3, #TWO
				
				CMP		 R2, #0b1000			 // check for KEY[3]
				MOVEQ	 R3, #THREE
				
				STR		 R2, [R0, #0xC]			 // reset edgecapture register
				
				LDR		 R2, [R1]
				EOR		 R3, R2					 // preserve the other HEX display
				STR		 R3, [R1]
